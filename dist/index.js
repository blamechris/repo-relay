...
function parseSessionLimitReset(error) {
        ...
        for (let attempt = 0;; attempt++) {
            try {
                ...
                if (attempt >= maxRetries) {
                    throw new Error(`Session limit retry exhausted after ${maxRetries} attempts. ` +
                ...
                if (waitMs > maxWaitMs) {
                    throw new Error(`Session limit exhausted. Resets at ${resetAt.toISOString()} (${Math.ceil(waitMs / 60_000)}min away), ` +
