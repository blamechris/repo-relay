...
| GitHub Expert | "GitHub Expert" | GitHub webhook payload handling, API reliability, event routing, rate limiting | Target involves GitHub webhook ingestion or event processing |
| SQLite Expert | "SQLite Expert" | Concurrency patterns, parameterized queries, transaction safety, better-sqlite3 best practices | Target involves state persistence or database operations |
...
- **Discord Expert** should weight embed compliance (title ≤256 chars, description ≤4096 chars, ≤25 fields), thread operation safety, and channel type guards.
- **GitHub Expert** should weight webhook payload robustness, event routing correctness per `mapGitHubEvent()` in cli.ts, and rate-limit handling.
- **SQLite Expert** should weight parameterized statement usage, transaction safety in StateDb, and concurrency patterns with better-sqlite3.
- **Guardian** should flag stale message handling edge cases, Discord permission checks, and webhook signature validation.
