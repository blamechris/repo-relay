...
- `npm ERR!` / `npm install failed` → RETRIGGER (transient npm registry)
- `TypeScript error` / `tsc` errors → FIX (type checking failure)
- `discord.js` / `DiscordAPIError` → RETRIGGER (Discord API transient)
- `better-sqlite3` / `database locked` → RETRIGGER (SQLite concurrency)
- `GitHub API rate limit` → RETRIGGER (transient)
