...
- `npm ERR!` with `ERESOLVE` → dependency conflict, check `package-lock.json` or run `npm install` locally
- `npm ERR! code E404` → missing package, verify package name and registry
- `npm ERR! code EACCES` → permission issue, check node_modules ownership
- `TypeScript error TS` → type check failure, run `npm run typecheck` locally to diagnose
- `Cannot find module` → missing dependency, run `npm install`
- `ENOENT: no such file or directory` → missing file or build artifact, check build step
