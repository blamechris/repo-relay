...
1. Discord embed titles are limited to 256 characters -- truncate with ellipsis for long PR titles
   Evidence: VERIFIED -- tested both approaches, long titles cause embed rejection
   Before/After: Pass full PR title to embed --> Truncate at 253 chars + "..."

1. Discord embed limits --> CLAUDE.md (## Discord Integration) -- awaiting approval

+ - Discord embed titles are limited to 256 characters. Truncate long PR titles at 253 characters with "..." suffix.
...
1. GitHub webhook payloads include null values for deleted branches -- check existence before accessing properties
   Evidence: VERIFIED -- delete event crashed with null reference error
   Before/After: Access branch properties directly --> Guard with null checks

2. SQLite better-sqlite3 requires explicit transaction for bulk inserts
   Evidence: OBSERVED -- saw it during debugging, did not isolate root cause

Persisted 1 of 2 insights.
1. GitHub webhook null handling --> .claude/rules/webhook-handling.md -- awaiting approval
2. SQLite transaction pattern --> skipped (already in CLAUDE.md line 84)

+ - GitHub webhook payloads include null values for deleted branches. Always check existence before accessing branch properties to prevent null reference errors.
...
```
User: /learn Discord rate limits apply per-channel, not globally -- use separate queues for different channels

1. Discord rate limiting scope --> .claude/rules/discord-embeds.md -- awaiting approval

+ Discord rate limits apply per-channel, not globally. Use separate message queues for different channels to maximize throughput.
...
1. Thread creation should use PR number as thread name, not title -- titles can exceed Discord's 100 char limit
   Evidence: VERIFIED -- long PR titles caused thread creation failures
   Before/After: Use PR title for thread name --> Use "PR #123" format for thread names

Persisted 0 of 1 insights (conflict found).
1. Thread naming pattern --> CONFLICTS with .claude/rules/discord-embeds.md line 3
   Existing: "Use PR title as thread name for easy identification"
   Found:    "Use PR number format to avoid Discord's 100 char thread name limit"
   Action needed: keep existing / replace / keep both
```
...
1. GitHub API secondary rate limits trigger on rapid sequential calls -- add 100ms delay between requests
   Evidence: VERIFIED -- got 403 secondary rate limit during bulk operations
   Before/After: Make rapid API calls --> Add delay between sequential requests

2. Currently working on webhook retry logic for PR #42

1. GitHub API rate limiting --> CLAUDE.md (## GitHub Integration) -- awaiting approval
2. Current WIP context --> CLAUDE.local.md -- applied

+ - GitHub API has secondary rate limits that trigger on rapid sequential calls. Add 100ms delay between requests to avoid 403 errors during bulk operations.
