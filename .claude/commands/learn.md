...
1. Discord embed title field must not exceed 256 characters or the embed silently fails to render
   Evidence: VERIFIED -- tested with 257-char title, embed dropped from message
   Before/After: Assume Discord accepts any title length --> Always truncate or validate embed titles to 256 chars max

1. Discord embed title limit --> .claude/rules/discord-embeds.md -- awaiting approval

+ - Discord embed titles must not exceed 256 characters. Longer titles cause the entire embed to fail silently. Always validate or truncate before building embeds.
...
2. SQLite concurrency pattern --> skipped (already in CLAUDE.md line 42)
...
```
User: /learn Thread stale message cleanup must check message.createdTimestamp, not message.id, because IDs are not monotonic across Discord shards

1. Thread stale message detection --> .claude/rules/thread-management.md -- awaiting approval

+ - When cleaning up stale messages in threads, use `message.createdTimestamp` for age comparison, not `message.id`. Message IDs are not monotonic across Discord shards and will produce incorrect age calculations.
...
1. Discord embed field count limit is 25, not 20 -- tested with 25 fields and embed rendered successfully
   Evidence: VERIFIED -- created embed with 25 fields, rendered in Discord client
   Before/After: Cap embed fields at 20 --> Allow up to 25 fields per Discord API spec

Persisted 0 of 1 insights (conflict found).
1. Discord embed field limit --> CONFLICTS with .claude/rules/discord-embeds.md line 5
   Existing: "Discord embeds support a maximum of 20 fields"
   Found:    "Discord embeds support a maximum of 25 fields"
   Action needed: keep existing / replace / keep both
```
...
1. GitHub webhook signature validation must use the raw request body, not parsed JSON, or HMAC verification fails
   Evidence: VERIFIED -- signature validation failed until raw body was used
   Before/After: Validate signature on parsed JSON --> Always validate on raw request body before parsing

2. Currently debugging thread message ordering issue in PR #123

1. GitHub webhook signature validation --> .claude/rules/webhook-handling.md -- awaiting approval
2. Current WIP context --> CLAUDE.local.md -- applied

+ - GitHub webhook signature validation must use the raw request body bytes, not the parsed JSON object. Parsing changes the byte representation and causes HMAC verification to fail.
