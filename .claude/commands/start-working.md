...
| Ready | Has both `complexity:low` or `complexity:medium` AND `testing:low` or `testing:medium` |
...
grep -r "TODO\|FIXME\|HACK\|XXX\|WORKAROUND" src/**/*.ts --include="*.ts" 2>/dev/null | grep -v node_modules | grep -v build | grep -v dist
```
...
| P1 | High | `complexity:low` + `testing:low`, `from-review` issues, milestone-assigned issues, stale PRs |
| P2 | Normal | `complexity:medium` + `testing:low/medium`, `from-audit` issues, roadmap items |
| P3 | Exploratory | `testing:high` (full GitHub Actions integration testing), codebase TODOs, vague issues without criteria, audit recommendations without issues |
...
**Ready to work on:**
- #18 — Add retry logic to API (`complexity:low`, `testing:low`) — has 3 acceptance criteria
- #25 — Improve error messages (`complexity:medium`, `testing:medium`) — milestone v1.2

**Blocked / Needs input:**
- #30 — Choose auth provider (`blocked`) — requires decision
...
|--------|-------|---------------|
| TODO | 12 | src/handlers/ (5), src/api/ (4), src/utils/ (3) |
| FIXME | 3 | src/handlers/webhook.ts (2), src/db/state.ts (1) |
| HACK | 1 | src/api/retry.ts:42 |
```
...
- **If you want to clean up:** The {N} TODOs in src/handlers/ suggest a refactoring pass
```
