...
- Every worker brief is **self-contained**: repo context, exact scope, house style, what NOT to touch, the return format, and the project's exploration shortcut (repo-relay has the `repo-memory` MCP — subagents should use `get_file_summary` / `search_by_purpose` / `get_related_files` / `get_project_map` before Read/grep to save tokens (subagents do not inherit CLAUDE.md, so name the MCP explicitly in the brief)).
