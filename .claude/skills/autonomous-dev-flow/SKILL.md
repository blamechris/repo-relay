...
- [ ] TypeScript type-checks clean (`npm run typecheck`)
...
```bash
# repo-relay has no UI/frontend surface — this phase never triggers (kept for template parity)
CHANGED_FILES=$(git diff --name-only main...HEAD)
if echo "$CHANGED_FILES" | grep -qE '^__NO_UI_SURFACE__$'; then
...
- **Attended runs** — end the session at the wave boundary; the user/orchestrator relaunches the next segment seeded from the handoff note (`$CLAUDE_BRIEF_DIR/../handoffs/repo-relay-<date>-handoff.md`) + the queue (`scratchpad/autonomous-queue.json`).
- **Unattended with a configured re-launcher** (none configured for this repo — no scheduled trigger, cron/launchd job, or `/loop` wrapper drives this skill today) — end the session; the re-launcher starts the next segment from the same seeds.
...
5. **Self-merge authority for this repo** — Merge only through the Unattended Merge Gate (`/unattended-merge`) — `/full-review` clean + ALL CI checks (`check (20)`, `check (22)`, `dist-freshness`) green on the final commit + ALL review threads resolved. No `gh pr merge --auto`, no GitHub auto-merge, no protection overrides. A failed gate means flag, don't merge. Every self-merged PR MUST appear as an entry in the final session report, and every merge is followed by retagging `v1` to main.
